...
def get_transcript(video_id, languages=['en', 'ru']):
    try:
        transcript = YouTubeTranscriptApi.get_transcript(video_id, languages=languages)
        text_blocks = [item['text'] for item in transcript]
        transcription = ' '.join(text_blocks)
...
def get_post_txt(yt, creator_name=False, post_lang='en'):
    try:
        video_id = yt.video_id
        languages = ['en', 'a.en', 'ru'] if post_lang == 'en' else ['ru', 'en', 'a.en']
        transcription = get_transcript(video_id, languages)
        if not transcription:
            raise ValueError("No subtitles available")
        
        # Optionally clean up the transcription as needed
    ...
    if creator_name:
        summary += f'\nThe original creator is - {creator_name}' if post_lang == 'en' else f'\nОригинальный создатель - {creator_name}'
...
def VideoToPost(link, post_lang='en', reference=False, post_img=False):
    ...
    for i_attempt in range(1,max_attempts+1):
        try:
            ...
            print(f'{i_attempt} attempt of get_post_txt failed because of {e}')
            time.sleep(5)
