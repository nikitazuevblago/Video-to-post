...
# SubFunctions
def get_post_txt(yt, audio_dir_path:str='yt_audio/'): # Get subtitles or generate from video's audio
    try:
...
def get_post_img(yt, img_path:str='images'):
    ...
    img_path = img_path / f'{len(os.listdir(img_path))}.jpg'
    image.save(img_path)
    return image


# MainFunction
def VideoToPost(link, post_dir='posts/'):
    yt = YouTube(link)
    yt.bypass_age_gate()
    post_txt = get_post_txt(yt)
    post_img = get_post_img(yt)
    overall_post = {'post_text':post_txt,'post_img':post_img}
    post_dir = Path(post_dir)
    post_dir.mkdir(parents=True, exist_ok=True)
    post_path = post_dir / f'{len(os.listdir(post_dir))}.pkl'
    if len(os.listdir(post_dir))==0:
